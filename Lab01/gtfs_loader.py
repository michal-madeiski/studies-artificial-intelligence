...
class GTFSLoader:
    ...
    def load_data(self) -> None:
        ...
        try:
            ...
            self.routes = pd.read_csv(f"{self.data_dir}/routes.txt", dtype={'route_id': str, 'agency_id': str})
            self.stops = pd.read_csv(f"{self.data_dir}/stops.txt", dtype={'stop_id': str, 'parent_station': str})
            self.trips = pd.read_csv(f"{self.data_dir}/trips.txt", dtype={'route_id': str, 'service_id': str, 'trip_id': str})
            self.calendar = pd.read_csv(f"{self.data_dir}/calendar.txt", dtype={'service_id': str})
            self.calendar_dates = pd.read_csv(f"{self.data_dir}/calendar_dates.txt", dtype={'service_id': str})
        ...
        try:
            ...
            self.stop_times = pd.read_csv(f"{self.data_dir}/stop_times.txt", dtype={'trip_id': str, 'stop_id': str})
            
            logger.debug("Parsing time_columns in stop_times...")
            self.stop_times['arrival_time_td'] = pd.to_timedelta(self.stop_times['arrival_time'])
            self.stop_times['departure_time_td'] = pd.to_timedelta(self.stop_times['departure_time'])
    ...
    def filter_data_for_date(self, target_date: datetime) -> tuple[pd.DataFrame, pd.DataFrame, dict]:
            ...
            try:
                ...
                active_stop_times = self.stop_times[self.stop_times['trip_id'].isin(active_trips['trip_id'])]
