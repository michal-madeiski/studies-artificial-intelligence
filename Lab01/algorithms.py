...
from settings import (
    PLATFORM_PLATFORM_TRANSFER_TIME,
    PLATFROM_STATION_TRANSFER_TIME,
    STATION_PLATFORM_TRANSFER_TIME,
    PLATFORM_PLATFORM_TRANSFER_STR,
    PLATFORM_STATION_TRANSFER_STR,
    STATION_PLATFORM_TRANSFER_STR,
    EARTH_RADIUS,
    TRAIN_SPEED,
    TRAIN_MAX_DISTANCE,
)
...
class PathFinder:
    ...
    def find_shortest_path_dijkstra(
        ...
        return self.__shortest_path_engine(
            start_stop_id, end_stop_id, start_time_str, max_wait_time, use_astar=False
        )
    ...
    def find_shortest_path_astar(
        ...
        if optimize_for == "t" or optimize_for == "p":
            ...
            logger.error(
                f"Invalid optimize_for value: '{optimize_for}'. Only 't' and 'p' are allowed."
            )
            raise ValueError()
    ...
    def __shortest_path_engine(
        ...
        logger.info(
            f"{algo_name} {opt_name}| Searching connection: {start_stop_id} -> {end_stop_id} [{start_time_str}]."
        )
        ...
        h_score = (
            self.__heuristic(self.graph.nodes[start_stop_id], target_node, optimize_for)
            if use_astar
            else 0
        )
        ...
        distances: dict[Any, Any] = {
            start_key: start_time if optimize_for == "t" else 0
        }
        ...
        while priority_queue:
            ...
            current_key = self.__get_node_key(
                current_node_id, current_trip_id, optimize_for
            )
            ...
            if current_node_id == end_stop_id:
                ...
                best_cost = (
                    self.__format_print_trip_total_time(total_travel_time)
                    if optimize_for == "t"
                    else current_transfers
                )
                info = "Time" if optimize_for == "t" else "Transfers"
                logger.success(
                    f"{algo_name} {opt_name}| Path found! {info}: {best_cost}."
                )
            ...
            if current_cost > distances.get(
                current_key, pd.Timedelta.max if optimize_for == "t" else float("inf")
            ):
                continue
            ...
            for edge in current_node.edges:
                if (
                    edge.route_name == STATION_PLATFORM_TRANSFER_STR
                    and current_node_id != start_stop_id
                ):
                    continue
                if (
                    edge.route_name == PLATFORM_STATION_TRANSFER_STR
                    and edge.target_id != end_stop_id
                ):
                    continue

                if edge.route_name in [
                    PLATFORM_PLATFORM_TRANSFER_STR,
                    PLATFORM_STATION_TRANSFER_STR,
                    STATION_PLATFORM_TRANSFER_STR,
                ]:
                    if edge.route_name == PLATFORM_PLATFORM_TRANSFER_STR:
                        transfer_time = PLATFORM_PLATFORM_TRANSFER_TIME
                    elif edge.route_name == PLATFORM_STATION_TRANSFER_STR:
                        transfer_time = PLATFROM_STATION_TRANSFER_TIME
                    else:
                        transfer_time = STATION_PLATFORM_TRANSFER_TIME
                ...
                next_key = self.__get_node_key(
                    edge.target_id, new_trip_id, optimize_for
                )
                cost_to_compare = next_time if optimize_for == "t" else new_transfers
                best_known_cost = distances.get(
                    next_key, pd.Timedelta.max if optimize_for == "t" else float("inf")
                )
                ...
                if cost_to_compare < best_known_cost:
                    ...
                    h = (
                        self.__heuristic(
                            self.graph.nodes[edge.target_id], target_node, optimize_for
                        )
                        if use_astar
                        else 0
                    )
                    ...
                    counter += 1
                    new_tuple = self.__build_priority_tuple(
                        optimize_for,
                        next_time,
                        new_transfers,
                        h,
    ...
    def __heuristic(
        self, node1: Node, node2: Node, optimize_for: str = "t"
    ) -> pd.Timedelta | int:
    ...
    def __reconstruct_path(came_from: dict, final_key: Any) -> list[dict]:
        ...
        while current_step in came_from:
            ...
            target_id = (
                current_step[0] if isinstance(current_step, tuple) else current_step
            )
    ...
    def print_trip(
        self, path: list[dict], total_time: pd.Timedelta, calc_time: float
    ) -> None:
        info = "ROUTE DETAILS"
        decoration = "=" * 60
        ...
        for step in path:
            if current_part is None:
                ...
                is_same_train = step.get("trip_id") is not None and step[
                    "trip_id"
                ] == current_part.get("trip_id")
                if is_same_train:
        ...
        for part in parts:
            ...
            if part["route_name"] == PLATFORM_PLATFORM_TRANSFER_STR:
                print(
                    f"[MOVE] {src.name} ({src_plat}) [{dep_str}] -> {tgt.name} ({tgt_plat}) [{arr_str}] | Platform change"
                )
            elif (
                part["route_name"] == PLATFORM_STATION_TRANSFER_STR
                or part["route_name"] == STATION_PLATFORM_TRANSFER_STR
            ):
                start_or_end = (
                    f"End"
                    if part["route_name"] == PLATFORM_STATION_TRANSFER_STR
                    else f"Start"
                )
                print(
                    f"[MOVE] {src.name} ({src_plat}) [{dep_str}] -> {tgt.name} ({tgt_plat}) [{arr_str}] | {start_or_end}"
                )
            else:
                print(
                    f"[RIDE] {src.name} ({src_plat}) [{dep_str}] -> {tgt.name} ({tgt_plat}) [{arr_str}] | Line: {part['route_name']}"
                )
